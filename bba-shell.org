...
#+BEGIN_SRC elisp
  ...
  (defun eshell/abbr-pwd ()
    ...
    (concat
    (with-face user-login-name :foreground "#ff383c")
    (with-face (concat "@" hostname) :foreground "#ffcc00")
    " "
...
Better than a specific cd command we can insert the current remote
host.

#+begin_src elisp
  (defun bba-eshell-host()
    (interactive)
    (let* ((d default-directory)
           (s (split-string d ":"))
           (path (car (last s)))
  ...
  (define-key eshell-mode-map (kbd "s-i") 'bba-eshell-host)
#+end_src
