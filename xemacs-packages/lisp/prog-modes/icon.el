...
;;; Synched up with: GNU Emacs 21.2, except that `font-lock-preprocessor-face'
;;; is used instead of `font-lock-builtin-face' and
;;; `font-lock-variable-name-face' instead of `font-lock-constant-face'.
...
with no args, if that value is non-nil."
  ...
  ;; XEmacs change: it isn't necessarily in XEmacs.
  (if (boundp 'hs-special-modes-alist)
  ...
  (eval-when-compile
    (list
     ;; Fontify procedure name definitions.
       '("^[ \t]*\\(procedure\\)\\>[ \t]*\\(\\sw+\\)?"
       ;; XEmacs change: builtin -> preprocessor.
       (1 font-lock-preprocessor-face) (2 font-lock-function-name-face nil t))))
   ...
   (eval-when-compile
	...
	'("&allocated" "&ascii" "&clock" "&col" "&collections" "&column"
	  ...
	  "&time" "&trace" "&ucase" "&version" "&window" "&x" "&y") t)
       ;; XEmacs change: constant -> variable-name.
       'font-lock-variable-name-face)
      ;; XEmacs change: this just causes too much trouble, commented out.
	...
	"^" (regexp-opt'("$dump" "$endif" "$else" "$include") t) "\\>" )
      ...
      (cons      ;; $warning $error
       (concat "^" (regexp-opt '("$warning" "$error") t)
	       "\\>[ \t]*\\(.+\\)?")
       ;; XEmacs change: builtin -> preprocessor.
       '((1 font-lock-preprocessor-face) (3 font-lock-warning-face nil t))))))
