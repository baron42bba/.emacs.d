...
;;     Do not work anymore for Emacsen <= 18
;;   Ftp access:
...
;;  command which  is  very dumb  for  creating its  compilation command (use
...
;;   - 1st looking at its name and extension (see variable
...
;;
;; @@ makefile-mode:
;;    The makefile is run with make through `compile' (user is prompted
...
;;   makefile-mode),  else try  to byte-recompile  all .el   files older than
...
(defgroup compilation-elisp nil
  "Emacs development compilation options."
  :group 'lisp
  :group 'compilation)
...
If you want to add or modify a COMPILE-FUNCTION and its associated
...
(defcustom mode-compile-filename-regexp-alist
  ...
  ;; and don't like these modes (prefer to edit these kind of
...
it), the MODE is only a pointer to an association in
`mode-compile-modes-alist' to get the COMPILE-FUNCTION and the
KILL-FUNCTION. The REGEXP could be a form which evaluate to a string.
  ...
  :type '(repeat
          (cons :tag "Association: filename/mode"
           (choice :tag "Filename regexp match"
            ...
            (sexp :tag "Form which evaluate to a string"))
           ;; I need to bind dynamically this with const, ideas??
           ;;`(choice
           ;; ,@(mapcar (lambda (x) `(const ,(car x))) mode-compile-modes-alist))))
           (function :tag "Mode to use -- should be a valid association in mode-compile-modes-alist --")))
  :group 'compilation)
...
the MODE is only a pointer to an association in `mode-compile-modes-alist'
...
This name could be used in your .Xdefaults or .Xresources file as:
...
;; XEmacs change: backwards compatibility.
;; APA: Make sure variable typo fix does not cause breakage!
;; Keep this before the associated defvar.
(define-obsolete-variable-alias
  'mode-compile-choosen-compiler
...
(defcustom mode-compile-remote-hosts-alist '()
  "Alist of favourite hosts names and the username
...
(defcustom cc-compilers-list '( "gcc" "c89" "acc" "cc" )
  "List of user's favourite C compilers in order of preference."
...
(defcustom cc-source-ext-list '( "c" )
  "Extensions for C compilable source files."
  :type '(repeat string)
  :group 'compile-c)
...
This could be any form evaluating to a string.
See `mode-compile-chosen-compiler' variable."
  :type '(choice
          string
          (sexp :tag "Form evaluating to a string"))
  :group 'compile-c)
...
(defcustom cc-source-file-ext-regexp "\\.c"
  "Regexp to find, from its name, if a C file is compilable."
  :type 'string
  :group 'compile-c)
...
(defcustom java-compilers-list '( "javac" )
  "List of user's favourite java compilers in order of preference."
  :type '(repeat (string :tag "Java Compiler name"))
  :group 'compile-java)

(defcustom java-companion-file-regexp ""
  "Regexp to find associated compilable Java companion file.

This is useless in Java because there do not exist uncompilable files."
  :type 'regexp
  :group 'compile-java)
...
(defcustom java-source-ext-list '( "java" )
  "Extensions for Java compilable source files."
  :type '(repeat string)
  :group 'compile-java)
...
`mode-compile-chosen-compiler' variable."
  :type '(choice
          string
          (sexp :tag "Form evaluating to a string"))
  :group 'compile-java)
...
(defcustom java-source-file-ext-regexp "\\.java"
  "Regexp to find, from its name, if a Java file is compilable."
  :type 'regexp
  :group 'compile-java)

(defcustom java-build-output-args nil
  "Don't build output-args for Java-mode."
  :type 'boolean
  :group 'compile-java)
...
(defcustom c++-companion-file-regexp "\\(_[Pp]\\)?\\.\\([pP]?[Hh][Hh]?\\|[Hh]\\+\\+?\\)"
  "Regexp to find associated compilable C++ companion file
from a header file."
  :type 'regexp
  :group 'compile-c++)
...
(defcustom ada-compilers-list
  '( "gcc" "gnat" "ada" )
  "List of user's favourite Ada compilers in order of preference."
  :type '(repeat (string :tag "Ada Compiler name"))
  :group 'compile-ada)

(defcustom ada-companion-file-regexp ""
  "Regexp to find associated compilable Ada companion file from a spec file.

This is useless in Ada because there do not exist uncompilable files."
  :type 'regexp
  :group 'compile-ada)
...
(defcustom ada-source-ext-list '( "ads" "adb" "ada" "a" )
  "Extensions for Ada compilable source files."
  :type '(repeat string)
  :group 'compile-ada)
...
(defcustom ada-source-file-ext-regexp "\\.\\(ad[abs]\\|a\\)"
  "Regexp to find, from its name, if an Ada file is compilable.
...
(defcustom f77-compilers-list '( "f77" "fc" )
  "List of user's favourite Fortran compilers in order of preference."
  :type '(repeat (string :tag "C Compiler name"))
...
(defcustom f77-source-ext-list '( "f" "F" "for" "For" )
  "Extensions for fortran compilable source files."
  :type '(repeat string)
  :group 'compile-fortran)
...
(defcustom f77-source-file-ext-regexp "\\.\\([Ff]\\|for\\)"
  "Regexp to find, from its name, if a fortran file is compilable."
  :type 'regexp
  :group 'compile-fortran)
...
;; XEmacs change: backwards compatibility.
;; APA: Moved this before the associated defcustom.
(define-obsolete-variable-alias
...
(defun mc--compile (compile-command)
  ...
  (if (null mc--remote-command)
    ...
    (let ((thisdir (expand-file-name (or default-directory "~"))))
      (mc--compile-sav
       (concat
        ;; The command to launch remote commands
        mc--remote-command
...
(cond
 ;; Check available frames functions
 ((fboundp 'make-frame)
  ;; GNU Emacs
...
(defsubst mc--funcall (command &rest params)
  ...
  ;; (maybe test window-system is set will be enough?).
  (cond ((and (eq mc--ws 'x)
...
(defun mc--which (file)
  ...
  (if mc--lucid-emacs-p
      ;; Some Emacsen don't have locate-file some have...
  ...
  (let ((file-name (or fname
    ...
    (cond
     ((string-match mc--source-ext-regexp file-name)
      ;; buffer is a compilable file
      (file-name-nondirectory file-name))

     ((setq pos (string-match mc--compfile-regexp file-name))
      ;; Buffer is not a compilable file, try to find associated
      ;; compilable file.
      (let ((tmp-ext-lst mc--source-ext-lst))
        (mc--msg "Looking for a compilable companion file for %s..."
                 (file-name-nondirectory file-name))
      ...
      (if found
          ;; A compilable companion source file found
          (file-name-nondirectory assoc-file)
        ;; No compilable companion source file found
        (mc--msg "Couldn't find any compilable companion file for %s ..."
                 (file-name-nondirectory file-name))
        nil))
    ...
    (save-excursion
      ...
      (save-excursion
        (save-restriction
          ...
          ;; THIS WILL NOT WORK FOR PROGRAMMING LANGUAGES
        ...
        (save-restriction
          ...
          (while (re-search-forward mc--makefile-rules-regexp nil t)
            (let ((rule (buffer-substring
              ...
              ;; and if not already in rules list.
              (if (and
  ...
  (let ((preference (or pref
    ...
    (cond
     ...
     (t
      ...
      (mc--msg "Invalid `mode-compile-preferred-default-makerule': '%s"
               mode-compile-preferred-default-makerule)
      (mc--makerule-completion alist outfile 'none)))))
    ...
    (if (setq mc--selected-makefile (mc--makefile-to-use))
      ...
      (or mc--compile-command
          (setq mc--compile-command
                (concat (setq mode-compile-chosen-compiler
                              (mc--find-compiler)) " "
...
(defun mc--lookat-name ()
  ;; Look at buffer file name to see if it can return a function
...
If no makefile is found try to run a C compiler on the file or its companion.
...
be specified by changing value of variable mode-compile-makefile-regexp) in
the current directory or not.
If no makefile is found try to run a Java compiler on the file or its
companion.
...
be specified by changing value of variable mode-compile-makefile-regexp) in
the current directory or not.
If no makefile is found try to run a C++ compiler on the file or its
companion.
...
be specified by changing value of variable mode-compile-makefile-regexp) in
the current directory or not.
If no makefile is found try to run a Fortran compiler on the file or
its companion..
...
If you want to step through errors set the variable `sh-compilation-error-regexp-alist'
...
If you want to step through errors set the variable `csh-compilation-error-regexp-alist'
to a value understandable by compile's `next-error'.
...
If you want to step through errors set the variable `zsh-compilation-error-regexp-alist'
to a value understandable by compile's `next-error'.
...
If you want to step through errors set the variable `perl-compilation-error-regexp-alist'
to a value understandable by compile's `next-error'.
...
If you want to step through errors set the variable `tcl-compilation-error-regexp-alist'
to a value understandable by compile's `next-error'.
...
If you want to step through errors set the variable `python-compilation-error-regexp-alist'
to a value understandable by compile's `next-error'.
...
Bound on \\[mode-compile]."
  ...
  (if (and mode-compile-always-save-buffer-p
           (buffer-file-name))
      ;; save-buffer already check if buffer had been modified
      (save-buffer))
