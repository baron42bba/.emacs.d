...
(if texinfo-mode-syntax-table
  ...
  (modify-syntax-entry ?\\ "." texinfo-mode-syntax-table)
  (modify-syntax-entry ?@ "_" texinfo-mode-syntax-table)
...
(defvar texinfo-font-lock-keywords
  `(;; All but the first had an OVERRIDE of t.
    ...
    ;; XEmacs change?? #### If so, we need to add text-clone-create from
    ;; 21.3
...
value of `texinfo-mode-hook'."
  ...
  ;; (setq imenu-case-fold-search nil) XEmacs deleted
  ...
  (if arg
      (progn
        ...
        (if (< arg 0)
          ...
          (forward-sexp arg)
          (insert "}")))
    ;; XEmacs change: operate over the active region if one is given
    (if (region-active-p)
...
to jump to the corresponding spot in the Texinfo source file."
  ...
  (let ((source-buffer (current-buffer))
    ...
    (pop-to-buffer "*Occur*")
    (goto-char (point-min))
    ;; #### Note: XEmacs removed the following line.  No comment about
    ;; this, nothing in the ChangeLog, so I'm leaving it as-is.  If
    ;; removing it did have some positive effects, then consider doing
    ;; this. --ben
...
;; XEmacs addition
;;;###autoload(add-to-list 'auto-mode-alist '("\\.texi\\(?:nfo\\)?\\'" . texinfo-mode))
