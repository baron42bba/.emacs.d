...
;;; Synched up with:  Version 1.32 in CVS
...
;; browse-url-w3m                     w3m         0.2.1, probably
;; browse-url-w3m-gnudoit             w3m remotely 0.2.1, probably
...
regexp should probably be \".\" to specify a default browser."
  :type '(choice
	  ...
	  (function-item :tag "Opera" :value browse-url-opera) ;; XEmacs
	  ...
	  ;; XEmacs: Emacs has an `alist' custom type here, but custom
	  ;; in XEmacs is buggy and cannot handle it, so we don't
...
(defcustom browse-url-filename-alist
  (\`					; Backquote syntax won't work.
   (("^/\\(ftp@\\|anonymous@\\)?\\([^:]+\\):/*" . "ftp://\\2/")
    ...
    ;; applies.
    ("^/\\([^:@]+@\\)?\\([^:]+\\):/*" . "ftp://\\1\\2/")
    (,@ (if (memq system-type '(windows-nt ms-dos cygwin32)) ;; XEmacs change
	    '(("^\\([a-zA-Z]:\\)[\\/]" . "file:\\1/")
              ("^[\\/][\\/]+" . "file://"))))
    ("^/+" . "file:/")))
...
(defcustom browse-url-gnudoit-program "gnuclient" ;; XEmacs
  "*The name of the `gnudoit' program.
Used by `browse-url-w3-gnudoit' and `browse-url-w3m-gnudoit'."
  :type 'string
  :group 'browse-url)

(defcustom browse-url-gnudoit-args '("-batch" "-eval") ;; XEmacs
...
(defcustom browse-url-temp-dir (temp-directory) ;; XEmacs change
...
(defun browse-url-url-at-point ()
  (let ((url (thing-at-point 'url)))
    (unless (zerop (length url)) ;; XEmacs bug workaround,
				 ;; `set-text-properties' in XEmacs <=
				 ;; 21.4.6 doesn't handle 0-0.
	  ...
	  (progn
		...
		(setq browse-url-temp-file-name
		      (convert-standard-filename
		       (make-temp-name ;; XEmacs doesn't have make-temp-file
...
(defun browse-url-default-windows-browser (url &optional new-window)
  ...
  (if (featurep 'xemacs) ;; XEmacs change
      (mswindows-shell-execute "open" url)
...
xterm, MMM, and then W3."
  (apply
    (cond
     ...
     ((fboundp 'w3m-goto-url-new-session) 'browse-url-w3m)
...
;;;###autoload
(defun browse-url-w3m-gnudoit (url &optional new-window)
  ;; new-window ignored
...
`browse-url-gnudoit-args'.  Default to the URL around or before point."
  (interactive (browse-url-interactive-arg "W3M URL: "))
    (apply 'start-process (concat "gnudoit:" url) nil
	   browse-url-gnudoit-program
	   (append browse-url-gnudoit-args
		   (list (concat "(w3m-goto-url-new-session \"" url "\")")
			 "(raise-frame)"))))
...
(defalias 'browse-url-kfm 'browse-url-kde) ;; XEmacs backwards compatibility
