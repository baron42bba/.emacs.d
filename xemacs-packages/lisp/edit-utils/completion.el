...
;; sjt sez: `let' overhead no longer matters in a command.
...
But only if it is longer than `completion-min-length'."
  ...
  (unwind-protect
      ;; Don't let local syntax tables override completion's.
      (let ((lookup-syntax-properties nil))
...
(defun add-completions-from-c-buffer ()
  ...
  (let (string next-point char
	(saved-syntax (syntax-table))
	;; This prevents us from using the information provided by the
	;; local syntax tables, but this function would have to be
	;; completely rewritten for that anyway.
  ...
  (if (file-writable-p filename)
	...
	(let* ((delete-old-versions t)
	  ...
	  (save-excursion
	    ...
	    (kill-buffer " *completion-save-buffer*"))
	  (cmpl-statistics-block
