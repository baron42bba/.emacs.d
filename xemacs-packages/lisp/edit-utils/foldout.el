...
;; XEmacs change.
(defun foldout-mouse-zoom (event click-count)
...
;;    (setq event (foldout-mouse-swallow-events event))
  ...
  (foldout-zoom-subtree
   (cond
    ((= click-count 1) -1)			; body only
    ((= click-count 2) '(1))		; subheadings only
    ((= click-count 3) nil)			; body and subheadings
    (t 0))))				; entire subtree

(defun foldout-mouse-show (event click-count)
...
;;  (setq event (foldout-mouse-swallow-events event))
  ...
  (foldout-mouse-goto-heading event)
  (cond
   ((= click-count 1) (show-entry))
   ((= click-count 2) (show-children))
   ((= click-count 3) (show-entry) (show-children))
   (t (show-subtree))))

(defun foldout-mouse-hide-or-exit (event click-count)
...
;;  (setq event (foldout-mouse-swallow-events event))
  ...
  (if (= click-count 1)
    ...
    (foldout-exit-fold
     (cond
      ((= click-count 2) 1)		; exit and hide
      ((= click-count 3) -1)		; exit don't hide
      (t 0)))))				; exit all

...
(meta control button{1,2,3}).

Valid modifiers are shift, control, meta, alt, hyper and super.")

;; XEmacs change.
(defun foldout-mouse-track-click-hook (event click-count)
  (if (and (memq click-count '(1 2 3 4))
	   (eq (event-modifier-bits
		(make-event 'button-release
			    `(button 1 modifiers ,foldout-mouse-modifiers)))
	       (event-modifier-bits event)))
      (let ((button (event-button event)))
	(cond ((eq button 1)
	       (funcall 'foldout-mouse-zoom event click-count))
	      ((eq button 2)
	       (funcall 'foldout-mouse-show event click-count))
	      ((eq button 3)
  ...
  (add-hook 'outline-mode-hook
	    (lambda ()
	      (add-hook 'mouse-track-click-hook
			'foldout-mouse-track-click-hook)))
  (add-hook 'outline-minor-mode-hook
	    (lambda ()
	      (add-hook 'mouse-track-click-hook
			'foldout-mouse-track-click-hook)))
  ;; XEmacs change
  (let ((mouse-1 (vector (append foldout-mouse-modifiers '(button1))))
	(mouse-2 (vector (append foldout-mouse-modifiers '(button2))))
	(mouse-3 (vector (append foldout-mouse-modifiers '(button3)))))

    (define-key outline-mode-map mouse-1 'mouse-track)
    (define-key outline-mode-map mouse-2 'mouse-track)
    (define-key outline-mode-map mouse-3 'mouse-track)

    (define-key outline-minor-mode-map mouse-1 'mouse-track)
