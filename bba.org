...
#+BEGIN_SRC emacs-lisp :tangle yes
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/async-20161103.1036")
        (require 'async)
        )
    )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/focus-20160111.522")
        (require 'focus)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (if (>= emacs-major-version 24)
      (progn
        ...
        (add-to-list 'load-path "~/.emacs.d/elpa/dash-20161116.920")
        (require 'smartparens)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (add-to-list 'load-path
               "~/.emacs.d/elpa/yasnippet-20180102.1824")
  (require 'yasnippet)
...
We use lisp in our snippets. So suppress warnings for that:
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (add-to-list 'load-path "~/.emacs.d/elpa/avy-20171230.220")
  (require 'avy)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun bba-create-ticket-tmp-dir()
    "create directory for the current ticket in tmp."
    (interactive)
    (org-narrow-to-subtree)
    (let ((beg (point)))
      (beginning-of-buffer)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (load-file "~/.emacs.d/elpa/org-mime-20170218.501/org-mime.el")
  (add-hook 'org-mode-hook
            (lambda()
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (add-to-list 'load-path "~/.emacs.d/elpa/org-sticky-header-20170422.2135" t)
  (add-hook 'org-mode-hook
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (add-to-list 'load-path "~/.emacs.d/elpa/twittering-mode-20160207.156")
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun bba-epa-list-packets ()
    ...
    (let ((bba-epa-filename (buffer-file-name)))
      (setq bba-epa-packets-buffer (generate-new-buffer "*Packets*"))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (add-to-list 'load-path "~/.emacs.d/elpa/sqlup-mode-20160911.1911")
  (require 'sqlup-mode)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (add-to-list 'load-path "~/.emacs.d/elpa/keyfreq-20160516.716")
  (require 'keyfreq)
  (keyfreq-mode 1)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (if (display-graphic-p)
	...
	(custom-set-faces
	 ...
	 '(org-block ((t (:inherit shadow :background "#29759c" :foreground "#84cbef"))))
	 )
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun bba-aws-desc-instances ( profile name)
    ...
    (insert-string (shell-command-to-string (concat "aws ec2 describe-instances --profile " profile " --filters \"Name=tag-key,Values=Name,Name=tag-value,Values=" name "*\" --query \"Reservations[].Instances[?State.Name=='running'].[InstanceId, PublicDnsName]\" --output table")))
    (message "done.")
  )
