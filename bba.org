...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  ;; Override the packages with the git version of Org and other packages
    ...
    (setq load-path
    (append
    (list (expand-file-name "~/.emacs.d/") (expand-file-name "~/.emacs.d/elpa/org-20200127"))
    ;;           (expand-file-name "/usr/share/emacs/lisp/")
    load-path))


  (add-to-list 'load-path "~/.emacs/elpa/org-20200127")
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (if (string-equal system-type "darwin")
          (progn
            ...
            (let ((path (shell-command-to-string ". ~/.bashrc >/dev/null; echo -n $PERL5LIB")))
            ...
            (let ((path (shell-command-to-string ". ~/.bashrc >/dev/null; echo -n $INFOPATH")))
              (setenv "INFOPATH" path))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/async-20200809.501")
        (require 'async)
        )
    )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/smartparens-20200105.1259")
        (add-to-list 'load-path "~/.emacs.d/elpa/dash-20210101.1712")
        (require 'smartparens)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (if (>= emacs-major-version 24)
      (when (display-graphic-p)
        (add-to-list 'load-path "~/.emacs.d/elpa/rainbow-delimiters-20191018.1233")
        ))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/expand-region-20200106.2058")
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (if (>= emacs-major-version 24)
      (progn
	(add-to-list 'load-path "~/.emacs.d/elpa/flycheck-20191126.2142")
	(require 'flycheck)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/ggtags-20190320.2208" t)
        (require 'ggtags)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (if (>= emacs-major-version 24)
      (progn
	(add-to-list 'load-path
                     "~/.emacs.d/elpa/helm-20191031.839")
	(require 'helm)
	(helm-mode 1)
...
In the end it turned out to be easy.

ssm doesn't support hostnames. just strange instance ids.
who can remmber those?
Luckily i am not the only one with a bad memory.
[[https://github.com/xen0l/aws-gate][aws-gate]] to the rescue!

Now we can run commands like

#+begin_src sh :eval never
  aws-gate session hostname
#+end_src

if those are configured in your aws-gate config file you don't have to
define aws account or region either.
Based on the telnet configuration (they both have similar
capabilities) I wrote this one:

#+begin_src elisp
  (add-to-list 'tramp-methods
    '("ssm"
      (tramp-login-program        "aws-gate")
      (tramp-login-args           (("session")("%h") ("2>/dev/null")))
