...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  ;; Override the packages with the git version of Org and other packages
    ...
    (setq load-path
    (append
    (list (expand-file-name "~/.emacs.d/") (expand-file-name "~/.emacs.d/elpa/org-9.7.17"))
    ;;           (expand-file-name "/usr/share/emacs/lisp/")
    load-path))


  (add-to-list 'load-path "~/.emacs/elpa/org-9.7.17")
          ...
          (progn
            ...
            (let ((infopath (shell-command-to-string ". ~/.bashrc 2>/dev/null >/dev/null; echo -n $INFOPATH")))
              ...
              (setq Info-directory-list
                    (append (mapcar #'expand-file-name (split-string-and-unquote infopath ":")) Info-directory-list)))
...
* print on osx
[2025-06-18 Wed 14:44]

#+begin_src sh :tangle ~/bin/lpr-ps
  #!/bin/bash
  uuid=$(uuidgen)
...
#+begin_src emacs-lisp
  (if (string-equal system-type "darwin")
      (if (file-exists-p "~/bin/lpr-ps")
            (setq ps-lpr-command "~/bin/lpr-ps")
          )
      )
#+end_src
* testing steampipe
[2023-04-05 Wed 13:18]

#+begin_src emacs-lisp :tangle never
  (defun bba-steampipe()
    "set sql connection for steampipe"
    (interactive)
    (setq sql-postgres-login-params nil)
    (let (foo (shell-command-to-string "steampipe service start")))
    (add-to-list 'sql-connection-alist
                 `(steampipe (sql-product 'postgres)
                             (sql-database ,(shell-command-to-string "steampipe service status --show-password | grep \"Connection string\" | awk '{print $3}' | tr -d '\n'")))))
#+end_src

* bba open safari url in eww
[2023-05-09 Tue 16:25]

[[file:elpa/org-mac-link-20220616.2340/org-mac-link.el::(org-mac-link-paste-applescript-links][org-mac-link]]

#+begin_src emacs-lisp :tangle never
  (defun bba-open-safari-in-eww ()
    "AppleScript to get the links to the frontmost window of the Safari.app and open in eww."
    (interactive)
...
#+begin_src emacs-lisp
  (add-to-list 'load-path "~/.emacs.d/elpa/f-20241003.1131")
  (require 'f)
#+end_src

* temporary load emacsql
[2023-05-06 Sat 12:04]

BUGBUG

#+begin_src sql emacs-lisp :tangle no
  (add-to-list 'load-path "~/.emacs.d/elpa/emacsql-20251130.1841")
  (require 'emacsql)

  (add-to-list 'load-path "~/.emacs.d/elpa/emacsql-sqlite-20230225.2205")
...
#+begin_src emacs-lisp
  (if (string-equal system-type "darwin")
          (progn
            (load-file (expand-file-name "~/.emacs.d/lisp/play-sound.el"))
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/async-20251005.634")
        (require 'async)
        )
    )
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        ...
        (add-to-list 'load-path "~/.emacs.d/elpa/popup-20250101.843")
        (require 'package)
        ;; (package-initialize)
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/smartparens-20250612.1050")
...
* docker
[2025-11-25 Tue 12:48]

#+begin_src emacs-lisp
    (require 'docker)
    (global-set-key "\C-cd" 'docker)
    (eval-after-load 'docker
      '(add-hook 'dockerfile-mode-hook '(lambda()   (define-key dockerfile-mode-map "\C-c\C-c" 'compile)
  (setq compile-command "docker build -t tag ."))))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        ...
        (defengine usenix
          "https://www.usenix.org/search/site/%s"
          :keybinding "u")

        (defengine vertica
          "https://docs.vertica.com/24.4.x/en/searchResults.html?search=%s"
          :keybinding "v")
...
* xah-html-decode
[2026-01-28 Wed 16:35]

#+begin_src emacs-lisp
  (defun xah-html-encode-ampersand-entity (&optional Begin End)
    "Replace HTML chars & < > \" to &amp; &lt; &gt; &quot; on current block or selection.
      ...
      (save-restriction
        ...
        (progn
          ...
          (while (re-search-forward "&\\|<\\|>\\|\"" nil t)
            (cond
             ((string-equal "&" (match-string 0)) (replace-match "&amp;" t t))
             ((string-equal "<" (match-string 0)) (replace-match "&lt;" t t))
             ((string-equal ">" (match-string 0)) (replace-match "&gt;" t t))
             ((string-equal "\"" (match-string 0)) (replace-match "&quot;" t t)))
  ...
  (defun xah-html-decode-ampersand-entity (&optional Begin End)
    "Replace &amp; &lt; &gt; &quot; to & < > ' on current block or selection.
...
#+BEGIN_SRC emacs-lisp
  (add-to-list 'load-path
               "~/.emacs.d/elpa/yasnippet-20250602.1342")
  (require 'yasnippet)
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
  	(add-to-list 'load-path "~/.emacs.d/elpa/flycheck-20251128.1706")
  	(require 'flycheck)
...
#+begin_src emacs-lisp
  (add-to-list 'load-path "~/.emacs.d/elpa/forge-20251201.1658")
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq tramp-ssh-controlmaster-options "-o ControlMaster=auto -o ControlPath='~/.ssh/sockets/ssh.%%r.%%n.%%p' -o ControlPersist=600"))
#+END_SRC
...
if you have multiple docker containers with the same name on different
hosts tramp tramp-default-proxies-alist gets confused.
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        ...
        (add-to-list 'load-path "~/.emacs.d/elpa/elfeed-org-20250219.950")
        (require 'elfeed-org)
        (elfeed-org)
...
#+BEGIN_SRC emacs-lisp
  (defun bba-epa-list-packets ()
    ...
    (let ((bba-epa-filename (buffer-file-name))
      ...
      (display-buffer bba-epa-packets-buffer)
   ))
#+END_SRC
...
If Emacs hangs while writing a gpg file we need this ignore wait
...
#+BEGIN_SRC emacs-lisp
  (add-to-list 'load-path "~/.emacs.d/elpa/auto-complete-20250101.843")
  (require 'auto-complete-config)
  (add-to-list 'ac-dictionary-directories "~/.emacs.d/elpa/auto-complete-20250101.843")
...
#+end_src
* gptel
[2026-01-29 Thu 10:14]

#+begin_src emacs-lisp
  (if (file-exists-p "/usr/local/bin/ollama")
      (progn (require 'gptel)
...
#+begin_src emacs-lisp
  (load-file (expand-file-name "~/.emacs.d/lisp/sql.el"))


  (setq sql-imenu-generic-expression
        (append sql-imenu-generic-expression '(("Projections" "^\\s-*create\\s-+\\(?:\\w+\\s-+\\)*\\(?:projection\\)\\s-+?\\(\\(?:\\w+\\s-*[_.]\\s-*\\)*\\w+\\)" 1))))
#+end_src
  ...
  (progn (defun my-sql-connect ()
    ...
    (let ((connection (helm-comp-read
      ...
      (if current-prefix-arg
          (progn (let ((conname (sql-generate-unique-sqli-buffer-name (cdr (car (alist-get 'sql-product (assoc 'smaatotst sql-connection-alist)))) connection )))(sql-connect connection conname)))
  ...
  (setq sql-connection-alist
        '((bbaprd (sql-product 'vertica)
          ...
          (mysql  (sql-product 'mysql)
                  ...
                  (sql-password "bar")
                  (sql-database "mysql")
                  (sql-mysql-options '("--init-command=SET AUTOCOMMIT=OFF")))))
...
#+BEGIN_SRC emacs-lisp
  ...
  (eval-after-load "python-mode"
    '(add-hook 'python-mode-hook (lambda() (add-hook 'write-contents-functions
                                                   (lambda()
                                                      (unless (boundp 'bba-keep-whitespaces)
                                                        (save-excursion
                                                          (delete-trailing-whitespace))))))))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun bba-insert-sql-date ()
  "Insert the current date as sql"
  (interactive)
...
#+BEGIN_SRC emacs-lisp
  ...
  (interactive)
  (if (use-region-p)
      (shell-command (concat "open dict://\"" (buffer-substring-no-properties (region-beginning)(region-end)) "\""))
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/ggtags-20230602.133" t)
        (require 'ggtags)
        (add-to-list 'load-path "~/.emacs.d/elpa/helm-gtags-20240925.755" t)
        (require 'helm-gtags)
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
	(add-to-list 'load-path
                     "~/.emacs.d/elpa/helm-20251212.555")
	(require 'helm)
	(helm-mode 1)
...
#+BEGIN_SRC emacs-lisp
  (if (>= emacs-major-version 24)
      (progn
        (add-to-list 'load-path "~/.emacs.d/elpa/helm-descbinds-20190501.935")
...
#+BEGIN_SRC emacs-lisp
  (add-to-list 'load-path "~/.emacs.d/elpa/keyfreq-20231107.106")
  (require 'json)
  (require 'keyfreq)
  (keyfreq-mode 1)
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key sql-mode-map (kbd "s-D") 'bba-insert-sql-date)
  (define-key sql-interactive-mode-map (kbd "s-D") 'bba-insert-sql-date)
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key bba/launcher "D" 'ediff-regions-linewise)
  ...
  (define-key bba/launcher "K" 'kubel)
  (define-key bba/launcher (kbd "s-k") 'kubed-transient)
...
* dired git
[2024-05-28 Tue 11:35]

#+begin_src emacs-lisp
  (setq dired-vc-rename-file  t)

  (defun git-add-files(files)
  "Run git add with the input FILES."
    (interactive)
...
To prevent Emacs from storing faces we have to overwrite the default
function:

#+begin_src emacs-lisp :tangle no
  (defun custom-set-faces (&rest args)
  "Overwrite default function to return just nil as we handle faces in orgmode."
  nil  )
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (if (string-equal system-type "darwin")
      (if (and (network-interface-info '"en0") (eql 203 (aref (car (cdr (network-interface-info '"en0"))) 2)))
...
#+BEGIN_SRC emacs-lisp
  (setq savehist-additional-variables '(kill-ring search-ring regexp-search-ring last-kbd-macro kmacro-ring shell-command-history register-alist projectile-compilation-cmd-map))
